                        ...
                        if r.get('menu_items'):
                            with st.expander(f"🍽️ View Menu ({len(r['menu_items'])} items)"):
                                for item in r['menu_items'][:15]:
                                    ...
                                    st.markdown(f"{veg_icon} **{item.get('name', 'Unknown')}** - ₹{price}")
...
# ==========================================
    ...
    try:
        ...
        if parent_filter != "All": params['parent'] = parent_filter
    ...
    if st.button("✅ Select Category"):
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get('status') == 'success': st.success(f"✅ {data.get('message')}")
                else: st.warning(data.get('message'))
            else:
